...
const callResultSeeds = ["No Answer","Answered","Interested","Not Interested","Busy","Wrong Number","Call Back"] as const;
...
async function initialize() {
  ...
  const callResultSeedMarker=await db.prepare("SELECT id FROM settings_entities WHERE kind='_seed_marker' AND title='call_results_v1'").first();
  if(!callResultSeedMarker) await db.batch([...callResultSeeds.map((title)=>db.prepare("INSERT INTO settings_entities (kind,title,is_active,custom_data) SELECT 'call_result',?,1,'{}' WHERE NOT EXISTS (SELECT 1 FROM settings_entities WHERE kind='call_result' AND LOWER(title)=LOWER(?))").bind(title,title)),db.prepare("INSERT INTO settings_entities (kind,title,is_active,custom_data) VALUES ('_seed_marker','call_results_v1',1,'{}')")]);
  await db.batch([
    ...
    db.prepare("CREATE INDEX IF NOT EXISTS call_records_direction_date_idx ON call_records (direction, call_at)"),
  ...
  const callStatusRulesMarker=await db.prepare("SELECT id FROM settings_entities WHERE kind='_seed_marker' AND title='call_status_rules_v3'").first();
  if(!callStatusRulesMarker){
    const calls=await db.prepare("SELECT id,phone,direction,call_at AS callAt,custom_data AS customData FROM call_records").all<{id:number;phone:string;direction:string;callAt:string;customData:string}>(),leads=await db.prepare("SELECT normalized_phone AS normalizedPhone,created_at AS createdAt FROM leads").all<{normalizedPhone:string;createdAt:string}>(),timeOf=(value:unknown)=>{const text=String(value??"").trim();return new Date(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(text)?`${text.replace(" ","T")}Z`:text).getTime()},earliestInboundByPhone=new Map<string,number>(),leadTimesByPhone=new Map<string,number[]>();
    for(const lead of leads.results){const time=timeOf(lead.createdAt),times=leadTimesByPhone.get(lead.normalizedPhone)||[];if(Number.isFinite(time))times.push(time);leadTimesByPhone.set(lead.normalizedPhone,times)}
    for(const call of calls.results){let details:Record<string,unknown>={};try{details=JSON.parse(call.customData||"{}")}catch{}if(call.direction!=="incoming"||details.followupSequence||details.inboundFollowupSequence||details.callSource==="imported_lead_history")continue;const phone=normalizePhone(call.phone),time=timeOf(call.callAt);if(phone&&Number.isFinite(time)&&time<(earliestInboundByPhone.get(phone)??Number.POSITIVE_INFINITY))earliestInboundByPhone.set(phone,time)}
    const updates:Array<ReturnType<typeof db.prepare>>=[];
    for(const call of calls.results){let details:Record<string,unknown>={};try{details=JSON.parse(call.customData||"{}")}catch{}const phone=normalizePhone(call.phone),callTime=timeOf(call.callAt),isOriginalInbound=call.direction==="incoming"&&!details.followupSequence&&!details.inboundFollowupSequence&&details.callSource!=="imported_lead_history",oldLead=isOriginalInbound&&Number.isFinite(callTime)&&(leadTimesByPhone.get(phone)||[]).some((leadTime)=>leadTime<callTime),oldStudent=isOriginalInbound&&!oldLead&&Boolean(details.registeredBefore),calledBefore=isOriginalInbound&&!oldLead&&!oldStudent&&Number.isFinite(callTime)&&(earliestInboundByPhone.get(phone)??Number.POSITIVE_INFINITY)<callTime,currentFinalStatus=String(details.finalStatus||"Not Yet"),finalStatus=oldLead?"Old Lead":oldStudent?"Old Student":calledBefore?"Called Before":["Called Before","Old Lead","Old Student"].includes(currentFinalStatus)?"Not Yet":currentFinalStatus;updates.push(db.prepare("UPDATE call_records SET custom_data=? WHERE id=?").bind(JSON.stringify({...details,oldLead,registeredBefore:oldStudent,calledBefore,finalStatus}),call.id))}
    for(let index=0;index<updates.length;index+=100)await db.batch(updates.slice(index,index+100));
    await db.prepare("INSERT INTO settings_entities (kind,title,is_active,custom_data) VALUES ('_seed_marker','call_status_rules_v3',1,?)").bind(JSON.stringify({updatedCalls:updates.length,appliedAt:new Date().toISOString()})).run()
  }
