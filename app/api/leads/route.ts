...
export async function POST(request: Request) {
  try {
    ...
    if (action === "createLead") {
      ...
      const customData = { ...((payload.customData as Record<string, unknown>) ?? {}), branch: payload.branch ?? "" };
      const result = await db.prepare(`INSERT INTO leads (full_name, primary_phone, normalized_phone, secondary_phone, email, source, campaign, interest, assigned_employee_id, priority, notes, custom_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .bind(fullName, primaryPhone, normalizedPhone, String(payload.secondaryPhone ?? ""), "", String(payload.source ?? "غير محدد"), "", String(payload.course ?? ""), assignee, "normal", String(payload.notes ?? ""), JSON.stringify(customData)).run();
    ...
    if (action === "recordLeadCallResult") {
      const id = Number(payload.id);
      const lead = await db.prepare("SELECT id, primary_phone AS phone, assigned_employee_id AS assignedEmployeeId FROM leads WHERE id=?").bind(id).first<{ id: number; phone: string; assignedEmployeeId: number | null }>();
      if (!lead) return Response.json({ error: "العميل غير موجود" }, { status: 404 });
      ...
      await db.prepare("INSERT INTO call_records (lead_id, phone, direction, result, assigned_employee_id, call_at, notes) VALUES (?, ?, 'outgoing', ?, ?, CURRENT_TIMESTAMP, ?)")
        .bind(lead.id, lead.phone, resultValue, lead.assignedEmployeeId, String(payload.notes ?? "")).run();
