...
export async function GET(request:Request) {
  try {
    ...
    const permissionRows=session?await db.prepare("SELECT page_key AS pageKey,can_view AS canView,data_scope AS dataScope FROM job_title_permissions WHERE job_title_id=?").bind(session.jobTitleId).all<{pageKey:string;canView:number;dataScope:string}>():{results:[]},pagePermission=page?(permissionRows.results.find((item)=>item.pageKey===page)||(page==="oralResults"?permissionRows.results.find((item)=>item.pageKey==="studentAttendance"):["studentsStatus","operationsRetention","retentionMoney","retentionTargets","operationsAbsenceReports","operationsAbsenceCalls"].includes(page)?permissionRows.results.find((item)=>item.pageKey==="studentsList"):["mtd","adsSpendingTargets","leadsCallsReport","leadsReport","callsReport"].includes(page)?permissionRows.results.find((item)=>item.pageKey==="marketingExpenses"):["financialReports","studentTransfers","trackTransfers"].includes(page)?permissionRows.results.find((item)=>item.pageKey==="payments"):undefined)):undefined,legacyFullAccess=!permissionRows.results.length||permissionRows.results.every((item)=>Boolean(item.canView)),pageAllowed=!session||!page||(pagePermission?Boolean(pagePermission.canView):legacyFullAccess),dataScope=pagePermission?.dataScope||"all";
    const today=cairoDateKey(new Date()),monthStart=`${today.slice(0,7)}-01`,requestedFrom=url.searchParams.get("from")||"",requestedTo=url.searchParams.get("to")||"",paymentFrom=requestedFrom||(["overview","financialReports","mtd","retentionMoney"].includes(page)?monthStart:today),paymentTo=requestedTo||today;
    ...
    const lightweightPage=paymentSummaryPage||page=="overview"||page=="operationsRetention"||page=="operationsAbsenceReports"||page=="operationsAbsenceCalls"||["groups","utilization","groupUtilization","floorSchedule","scheduleFinal","studentAttendance","studentAbsence","oralResults"].includes(page);
    ...
    if(session&&dataScope!=="all"){
      ...
      }else if(["studentsList","studentPlacement","studentComplaints","studentInformations","studentMisplaced","studentReported","studentsStatus","operationsRetention","retentionMoney","operationsAbsenceReports","operationsAbsenceCalls","studentTransfers","trackTransfers"].includes(page)||page==="overview"){
...
export async function POST(request: Request) {
  try {
    ...
    if(action==="createStudent"||action==="updateStudent"){
      ...
      const customData=JSON.stringify(payload.customData??{}),email=String(payload.email??""),trackId=Number(payload.trackId)||null,branchId=Number(payload.branchId)||null,status=String(payload.status??"active");
      if(action==="updateStudent"){
        ...
        if(createdAt)await db.prepare("UPDATE students SET full_name=?,mobile=?,secondary_mobile=?,email=?,level_id=?,track_id=?,branch_id=?,status=?,custom_data=?,created_at=? WHERE id=?").bind(fullName,mobile,secondaryMobile,email,levelId,trackId,branchId,status,customData,createdAt,id).run();
        else await db.prepare("UPDATE students SET full_name=?,mobile=?,secondary_mobile=?,email=?,level_id=?,track_id=?,branch_id=?,status=?,custom_data=? WHERE id=?").bind(fullName,mobile,secondaryMobile,email,levelId,trackId,branchId,status,customData,id).run();
        return Response.json({ok:true})
      }
      const result=await db.prepare("INSERT INTO students (full_name,mobile,secondary_mobile,email,level_id,track_id,branch_id,status,custom_data) VALUES (?,?,?,?,?,?,?,?,?)").bind(fullName,mobile,secondaryMobile,email,levelId,trackId,branchId,status,customData).run(),studentId=Number(result.meta.last_row_id),normalizedMobile=normalizePhone(mobile);
      ...
      const originTypes=[linkedLeads.results.length?"Lead":"",linkedCalls.some((call)=>call.direction==="incoming")?"Inbound Call":""].filter(Boolean),studentDetails={...((payload.customData&&typeof payload.customData==="object"?payload.customData:{}) as Record<string,unknown>),createdById:sessionEmployee?.id||null,createdBy:sessionEmployee?.fullName||"System",originTypes,linkedLeadIds:linkedLeads.results.map((lead)=>lead.id),linkedCallIds:linkedCalls.map((call)=>call.id)};updates.push(db.prepare("UPDATE students SET custom_data=? WHERE id=?").bind(JSON.stringify(studentDetails),studentId));if(updates.length)await db.batch(updates);
    ...
    if(action==="transferPayment"){
      ...
      const createdAt=new Date().toISOString(),paymentType=transferType==="student"?"Transfer to another student":"Transfer to another Track",targetTrackName=targetTrack?.title||(await db.prepare("SELECT title FROM tracks WHERE id=?").bind(targetStudent.trackId).first<{title:string}>())?.title||"",transferDetails={transferType,paymentType,sourcePaymentId:id,main:details.main||"",invoice:details.invoice||"",sourceStudentId:record.studentId,sourceStudentName:record.studentName,sourceMobile:record.mobile,sourceTrackId:record.trackId,sourceTrackName:record.trackName||"",targetStudentId:targetStudent.id,targetStudentName:targetStudent.fullName,targetMobile:targetStudent.mobile,targetTrackId:targetTrack?.id||targetStudent.trackId||null,targetTrackName,amount:Number(amount.toFixed(2)),reason,createdById:sessionEmployee.id,createdByName:sessionEmployee.fullName,createdAt,excludedFromRevenue:true};
      const inserted=await db.prepare("INSERT INTO student_records (student_id,kind,record_date,status,notes,custom_data) VALUES (?,'payment_transfer',?,'Transferred',?,?)").bind(record.studentId,createdAt,reason,JSON.stringify(transferDetails)).run(),transferredAmount=Number((Number(details.transferredAmount||0)+amount).toFixed(2)),history=Array.isArray(details.transferHistory)?details.transferHistory:[];await db.prepare("UPDATE student_records SET custom_data=? WHERE id=?").bind(JSON.stringify({...details,transferredAmount,transferHistory:[...history,{transferRecordId:Number(inserted.meta.last_row_id),...transferDetails}]}),id).run();return Response.json({id:inserted.meta.last_row_id,paymentType,mainPaidTotal,available:Number((available-amount).toFixed(2))},{status:201});
    }
    ...
    if(action==="addGroupStudent") {
      ...
      let bookedLevels=0;for(const row of paymentRows.results){try{const payment=JSON.parse(row.customData||"{}") as Record<string,unknown>;if(payment.isMainPayment&&!payment.voided)bookedLevels+=Math.max(0,(Number(payment.levels??offerLevels.get(Number(payment.offerId))??0)||0)-Number(payment.refundedLevels||0))}catch{}}
