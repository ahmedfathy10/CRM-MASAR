...
type SessionEmployee = { id: number; fullName: string; phone: string; status: string; jobTitle: string | null };
...
export async function POST(request: Request) {
  ...
  const phone = String(payload.phone ?? "").trim(); const password = String(payload.password ?? "");
  if (!phone || !password) return Response.json({ error: "أدخل رقم الموبايل وكلمة المرور" }, { status: 400 });
  const employee = await db.prepare("SELECT e.id, e.full_name AS fullName, e.phone, e.status, e.password_hash AS passwordHash, j.name AS jobTitle FROM employees e LEFT JOIN job_titles j ON j.id=e.job_title_id WHERE e.phone=? LIMIT 1").bind(phone).first<SessionEmployee & { passwordHash: string }>();
  ...
  return Response.json({ employee: safeEmployee }, { headers: { "Set-Cookie": cookie(token, 60 * 60 * 24 * 30) } });
}
