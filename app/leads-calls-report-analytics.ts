...
export type AcquisitionOrigin={
  kind:"Lead"|"Call";id:number;date:string;name:string;phone:string;source:string;campaign:string;
...
function prepareLeadsCallsAnalytics(data:LeadsCallsAnalyticsInput):PreparedLeadsCallsAnalytics{
  ...
  for(const origin of originDrafts){const phone=normalizeEgyptPhone(origin.phone);if(phone)originsByPhoneDraft.set(phone,[...(originsByPhoneDraft.get(phone)||[]),origin]);for(const studentId of origin.explicitStudentIds)explicitOriginsByStudent.set(studentId,[...(explicitOriginsByStudent.get(studentId)||[]),origin])}
  ...
  for(const origin of origins){
    ...
    const phone=normalizeEgyptPhone(origin.phone);if(phone)originsByPhone.set(phone,[...(originsByPhone.get(phone)||[]),origin]);
  }
  ...
  const findAttribution=(row:PaymentRow)=>{
    ...
    for(const id of unique([row.detail.leadId,...(Array.isArray(row.detail.linkedLeadIds)?row.detail.linkedLeadIds:[]),...(Array.isArray(studentDetail.linkedLeadIds)?studentDetail.linkedLeadIds:[])]))add(originByKey.get(`Lead:${id}`),"Explicit Link","High",3);
    for(const id of unique([row.detail.callId,...(Array.isArray(row.detail.linkedCallIds)?row.detail.linkedCallIds:[]),...(Array.isArray(studentDetail.linkedCallIds)?studentDetail.linkedCallIds:[])]))add(originByKey.get(`Call:${id}`),"Explicit Link","High",3);
...
export function buildLeadsCallsAnalytics(data:LeadsCallsAnalyticsInput,filters:LeadsCallsFilters):LeadsCallsAnalytics{
  ...
  const monthly=(kind:"Lead"|"Call"):MonthlyChannelRow[]=>{
    const rows=months.map((month)=>{const channelReceipts=receipts.filter((row)=>row.originType===kind&&row.paymentDate.slice(0,7)===month),channelMains=mains.filter((main)=>main.originType===kind&&dateKey(main.record.recordDate).slice(0,7)===month),channelPaid=channelReceipts.reduce((sum,row)=>sum+row.paid,0),channelOffer=channelMains.reduce((sum,row)=>sum+row.offerValue,0);return {month,paid:channelPaid,receipts:channelReceipts.length,registrations:new Set(channelMains.map((main)=>main.record.studentId)).size,offerValue:channelOffer,collectionRate:channelOffer?channelPaid*100/channelOffer:0,share:0}});
    const totalPaid=rows.reduce((sum,row)=>sum+row.paid,0);
    return rows.map((row)=>({...row,share:totalPaid?row.paid*100/totalPaid:0}));
  };
